...
public class NormalTerrainGen 
{
    ...
    public NormalTerrainGen()
    {
    	this.heightMap = new double[255];
    ...
    public void setup(World world, Random rand)
    {
        ...
        ContextOverworld context = new ContextOverworld(minLimitPerlinNoise, maxLimitPerlinNoise, mainPerlinNoise, surfaceNoise, noiseGen5, depthNoise, mobSpawnerNoise);
    ...
    {
    	this.depthRegion = this.depthNoise.generateNoiseOctaves(depthRegion, x, z, 5, 5, 200.0D, 200.0D, 0.5D);
    	this.mainNoiseRegion = this.mainPerlinNoise.generateNoiseOctaves(mainNoiseRegion, x, y, z, 5, 33, 5, 8.55515D, 4.277575D, 8.55515D);
    	this.minLimitRegion = this.minLimitPerlinNoise.generateNoiseOctaves(minLimitRegion, x, y, z, 5, 33, 5, 684.412D, 684.412D, 684.412D);
    	this.maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseOctaves(maxLimitRegion, x, y, z, 5, 33, 5, 684.412D, 684.412D, 684.412D);
    	...
    	for(int j = 0; j < 5; j++)
    	{
    		for(int k = 0; k < 5; k++)
    		{
    			...
    			for(int l = -b0; l <= -b0; l++)
    			{
    				for(int m = -b0; m <= -b0; m++)
    				{
    					Biome biome = this.biomesForGeneration[j + 2 + (k + 2) * 10];
                        float baseHeight = biome.getBaseHeight();
                        float variation = biome.getHeightVariation();
                        
                        float f5 = biomeWeights[l + 2 + (m + 2) * 5] / (baseHeight + 2.0F);
                        f += variation * f5;
                        f1 += baseHeight * f5;
                        f2 += f5;
    				}
                ...
                double d12 = this.depthRegion[b] / 8000.0D;
                ...
                for (int n = 0; n < 33; n++) 
                {
                    double d6 = (n - d5) * 12.0D * 128.0D / 256.0D / d14;
                    if(d6 < 0.0D) d6 *= 4.0D;
                    
                    
                    double d7 = this.minLimitRegion[a] / 512.0D;
                    double d8 = this.maxLimitRegion[a] / 512.0D;
                    double d9 = (this.mainNoiseRegion[a] / 10.0D + 1.0D) / 2.0D;
                    double d10 = MathHelper.clamp(d7, d8, d9) - d6;
                    
                    if(n > 29) 
                    {
                    	double d11 = ((n - 29) / 3.0F);
                    	d10 = d10 * (1.0D - d11) + -10.0D * d11;
                    }
                    
                    this.heightMap[a] = d10;
                    a++;
                }
    		}
    	}
                        ...
                        for (int x = 0; x < 4; ++x) {
                            ...
                            for (int z = 0; z < 4; ++z) {
                                if (height < 2) {
                                    ...
                                    primer.setBlockState(x4 * 4 + x, height32 * 8 + h, z4 * 4 + z, BlockInit.ore.getDefaultState().withProperty(CustomBlockOre.VARIANT, CustomBlockPlanks.EnumType.COPPER));
                                }
                            }
    ...
    {
    	if(ForgeEventFactory.onReplaceBiomeBlocks(generator, x, z, primer, world)) return;
    	this.depthBuffer = this.surfaceNoise.getRegion(depthBuffer, (x*16), (z*16), 16, 16, 0.0625D, 0.0625D, 1.0D);
